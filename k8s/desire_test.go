...
var _ = Describe("Desire", func() {
	...
	Context("When updating an LRP", func() {
		...
		It("should update the instance", func() {
			Expect(instanceManager.UpdateCallCount()).To(Equal(1))
			updateLrp := instanceManager.UpdateArgsForCall(0)
			Expect(updateLrp).To(Equal(lrp))
		})

		It("should update the ingress", func() {
			Expect(ingressManager.UpdateCallCount()).To(Equal(1))

			updateLRP := ingressManager.UpdateArgsForCall(0)
			Expect(updateLRP).To(Equal(lrp))
		})
		...
		Context("When updating the ingress fails", func() {

			BeforeEach(func() {
				ingressManager.UpdateReturns(errors.New("doing"))
			})
