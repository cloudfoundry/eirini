...
var _ = Describe("RouteStatefulSetInformer", func() {
	...
	When("the config file doesn't exist", func() {
		It("exits reporting missing config file", func() {
			session = eiriniBins.RouteStatefulsetInformer.Restart("/does/not/exist", session)
	...
	When("the config file is not valid yaml", func() {
		It("exits reporting missing config file", func() {
			session = eiriniBins.RouteStatefulsetInformer.Restart(pathToTestFixture("invalid.yml"), session)
			Eventually(session).Should(gexec.Exit())
			Expect(session.ExitCode).ToNot(BeZero())
