...
func main() {
	...
	log := lager.NewLogger("resource-validator")
	...
	mgr.GetWebhookServer().Register("/lrps", &admission.Webhook{
		Handler: webhook.NewLRPResourceValidator(log, decoder),
	})
...
func readConfigFile(path string) (*eirini.ResourceValidatorConfig, error) {
	fileBytes, err := ioutil.ReadFile(filepath.Clean(path))
	if err != nil {
		return nil, errors.Wrap(err, "failed to read file")
	}

	var conf eirini.ResourceValidatorConfig
